...
contract AMM is AccessControl{
    ...
    */
    function tradeTokens( address sellToken, uint256 sellAmount ) public {
        ...
        // Apply fee on input side (basis points)
        uint256 feeFactor = 10000 - feebps;
        uint256 amountInWithFee = sellAmount * feeFactor;
        ...
        if (sellToken == tokenA) {
            ...
            emit Swap(tokenA, tokenB, sellAmount, amountOut);
        } else {
            // Selling B for A
            ...
            emit Swap(tokenB, tokenA, sellAmount, amountOut);
        }
    ...
    */
    function provideLiquidity( uint256 amtA, uint256 amtB ) public {
        ...
        // Ensure both sides are non-zero on first initialization so invariant is meaningful
        if (invariant == 0) {
            require(
